...
export const getAllCategories = async (
  ...
  try {
    ...
    const categoryList = await new CategoriesGetAll().execute(restaurant.id);
...
export const getAllCategoriesAndProducts = async (
  ...
  try {
    const { restaurantId } = req.params;
    const restaurant: any = await new RestaurantGet().execute(restaurantId); // TODO: tipar
    const categoriesAndProducts = await new CategoriesGetAllWithProducts().execute(restaurant.id);
    return res.status(200).json(categoriesAndProducts);
  } catch (error) {
    console.log("🚀 ~ file: get.ts:9 ~ list ~ error:", error);
  }
};
