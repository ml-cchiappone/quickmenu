...
export class ProjectService {
  ...
  private _url = `${BACKEND_URL}/project`;
  ...
  get(id: number): Observable<ProjectModel> {
    ...
    return this._http.get<ProjectModel>(url).pipe(catchError(this.handleError));
  }

  create(data: any): Observable<ProjectModel> {
    const url = `${this._url}/`;
    return this._http.post<ProjectModel>(url, data).pipe(catchError(this.handleError));
  }
  ...
  update(id: number, data: any): Observable<ProjectModel> {
    const url = `${this._url}/${id}`;
    return this._http.patch<ProjectModel>(url, data).pipe(catchError(this.handleError));
  }

  getList(limit: number, offset: number, order: string, orderBy: string, userId: number, title: string, state: string): Observable<GridModel<ProjectModel>> {
    ...
    return this._http.get<GridModel<ProjectModel>>(url).pipe(catchError(this.handleError));
  }
